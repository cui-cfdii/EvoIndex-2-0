...
console.log('P2 召回率验证测试 - 增加样本文章后');
console.log('='.repeat(60));
...
for (const sample of testSamples) {
  ...
  const fileMatched = sample.expectedFile === bestMatch?.filename || 
                      bestMatch?.filepath.includes(sample.expectedFile);
...
console.log('   提升幅度：+' + ((avgRecall - 0.917) * 100).toFixed(1) + '%');

// 保存结果
const reportPath = join(PROJECT_ROOT, 'test', 'p2_recall_validation_report.json');
writeFileSync(reportPath, JSON.stringify({
  version: 'v5-jieba-cmaes-optimized-with-articles',
