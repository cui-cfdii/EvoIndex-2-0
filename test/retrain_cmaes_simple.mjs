...
console.log('CMA-ES 重新训练 - 简化版 (14 个文档)');
console.log('='.repeat(60));
...
function evaluate(weights) {
  ...
  for (const doc of documents) {
    ...
    const matchedTokens = tokens.filter(token => {
      const idx = topTerms.indexOf(token);
      if (idx === -1) return false;
      return weights[idx] > 0.3;
    });
    
    const coverage = matchedTokens.length / Math.max(1, tokens.length);
...
writeFileSync(newDictPath, JSON.stringify({
  version: 'v6-cmaes-retrained-14docs-simple',
